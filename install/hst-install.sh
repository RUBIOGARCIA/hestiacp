...
check_wget_curl(){
    ...
    if [ -e '/usr/bin/wget' ]; then
        wget -q https://raw.githubusercontent.com/RUBIOGARCIA/hestiacp/master/install/hst-install-debian.sh -O hst-install-debian.sh
        if [ "$?" -eq '0' ]; then
    ...
    if [ -e '/usr/bin/curl' ]; then
        curl -s -O https://raw.githubusercontent.com/RUBIOGARCIA/hestiacp/master/install/hst-install-debian.sh
        if [ "$?" -eq '0' ]; then
            bash hst-install-$type.sh $*
            exit
        else
